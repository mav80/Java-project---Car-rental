...
public interface OrderRepository extends JpaRepository<Order, Long>{
	...
	@Query(value = "SELECT * FROM orders JOIN users ON users_id=users.id WHERE users.username LIKE %?1% ORDER BY created", nativeQuery = true)
	List<Order> customFindOrdersByUserName(String name);
	
	@Query(value = "SELECT * FROM orders JOIN users ON users_id=users.id WHERE users.email = ?1 ORDER BY created", nativeQuery = true)
